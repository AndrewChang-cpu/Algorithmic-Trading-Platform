...
def _run_task(monkeypatch, dsn, job_id, s3_key, strategy_code, lean_side_effect):
    """Run the backtest task with moto S3 and mocked lean_runner."""
    import celery_worker

    # Patch DB and S3 config on the module via monkeypatch (restores automatically)
    monkeypatch.setattr(celery_worker, "DATABASE_URL", dsn)
    ...
    with patch("celery_worker.run_lean_backtest", side_effect=lean_side_effect), patch(
        "celery_worker.requests.post", return_value=_mock_requests_post()
    ):
        celery_worker.run_lean_backtest_task.apply(args=[job_id])


@mock_aws
def test_happy_path(monkeypatch, pg_dsn, tmp_path):
    data = _seed(pg_dsn)
    job_id = data["job_id"]
    ...
    assert (
        port_count >= 1
    ), f"expected at least 1 portfolio_metrics row, got {port_count}"
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT sharpe_ratio FROM performance_metrics WHERE job_id=%s",
                (job_id,),
            )
...
@mock_aws
def test_strategy_validation_failure(monkeypatch, pg_dsn, tmp_path):
    ...
    s3.create_bucket(Bucket="atp-strategies")
    s3.put_object(
        Bucket="atp-strategies", Key=data["s3_key"], Body=INVALID_STRATEGY.encode()
    )
    ...
    assert (
        "os" in job["error_message"].lower() or "import" in job["error_message"].lower()
    )


@mock_aws
def test_lean_timeout(monkeypatch, pg_dsn, tmp_path):
    data = _seed(pg_dsn)
    job_id = data["job_id"]
    _seed_market_data(pg_dsn)
    ...
    _run_task(
        monkeypatch, pg_dsn, job_id, data["s3_key"], data["strategy_code"], timeout_mock
    )
...
def test_invalid_uuid_no_db_call():
    ...
    with patch(
        "celery_worker._get_db",
        side_effect=AssertionError("_get_db must not be called"),
    ):
...
@mock_aws
def test_lean_runtime_error(monkeypatch, pg_dsn, tmp_path):
    data = _seed(pg_dsn)
    job_id = data["job_id"]
    _seed_market_data(pg_dsn)
    ...
    mock_exc = RuntimeError("container exited 1")
    ...
    job = _get_job(pg_dsn, job_id)
    assert job["status"] == "failed"
    assert str(mock_exc) in job["error_message"]
