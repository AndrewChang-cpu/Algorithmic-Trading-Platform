...
def test_daily_bar_price_scaling(tmp_path):
    rows = [{
        ...
        "open": 150.25, "high": 151.00, "low": 149.50, "close": 150.75, "volume": 1000000
    }]
...
def test_minute_bar_milliseconds(tmp_path):
    rows = [{
        ...
        "open": 100.0, "high": 100.5, "low": 99.5, "close": 100.25, "volume": 5000
    }]
...
def test_directory_structure(tmp_path):
    rows = [{"time": datetime(2024, 3, 15, tzinfo=timezone.utc),
...
def test_multiple_dates(tmp_path):
    rows = [
        {"time": datetime(2024, 1, 2, tzinfo=timezone.utc), "open": 1.0, "high": 1.0, "low": 1.0, "close": 1.0, "volume": 100},
        {"time": datetime(2024, 1, 3, tzinfo=timezone.utc), "open": 2.0, "high": 2.0, "low": 2.0, "close": 2.0, "volume": 200},
    ]
    materialize_lean_csv(rows, str(tmp_path), "SPY", "1d")
